...
export default function MovieCards(props){
    ...
    let newMovieCards = props.movies.map((e,i)=>{
        return (
            <Card key={i} sx={{maxHeight:'150px',minWidth:'275px'}} className="mx-3">
                    <CardMedia
                        component="img"
                        ...
                        className="transition ease-out duration-200 hover:scale-110 hover:cursor-pointer pointer-events-auto"
                    />
    ...
    return(
        <Box ref={horizontalScrollRef} onWheel={handleScroll} component="div" 
            className="fixed bottom-0 flex items-center w-full overflow-auto overflow-x-hidden"
            sx={{mb:1, ml:3, height:'200px'}}
        >
            {newMovieCards}
        </Box>
    )
